...
var damlev = require('damlev').default;
...
function bktree (term, options) {
  if (options && options.transposition) this.stringCompare = damlev;
  else this.stringCompare = levenshtein;
...
bktree.prototype._addTerm = function (newTerm) {
  var dist = this.stringCompare(this.term, newTerm);
...
bktree.prototype.query = function (queryTerm, maxDist, k) {
  ...
  for (var ii = 0; ii < len; ++ii) {
    results.push(this.details ? tempResults[ii] : tempResults[ii].t);
  }
  return results;
};

bktree.prototype._query = function (queryTerm, maxDist, d, results) {
  var dist = this.stringCompare(this.term, queryTerm);
  if (dist <= maxDist) {
